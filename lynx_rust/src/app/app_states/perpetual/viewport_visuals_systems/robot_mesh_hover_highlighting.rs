...
pub fn robot_mesh_hover_highlighting(mut event: EventReader<PickingEvent>,
    ...
    for ev in event.iter() {
        ...
        match picking_event {
            ...
            PickingEvent::Hover(h) => {
                ...
                if q.is_ok() {
                    let q = q.unwrap();
                    let curr_material = robot_set_entity_server.get_curr_material(q.robot_server_vector_idx, q.robot_set_idx, q.robot_link_idx).expect("error");
                    if curr_material <= LynxMaterialType::Hover {
                        match h {
                            HoverEvent::JustEntered(e) => {
                                robot_set_entity_server.change_link_material_data(q.robot_server_vector_idx, q.robot_set_idx, q.robot_link_idx, LynxMaterialType::Hover);
                            }
                            HoverEvent::JustLeft(e) => {
                                robot_set_entity_server.reset_link_material_data(q.robot_server_vector_idx, q.robot_set_idx, q.robot_link_idx);
                            }
                        }
                    }
                }
