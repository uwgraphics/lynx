...
pub fn robot_mesh_focus_highlighting(mut robot_set_entity_server: ResMut<RobotSetEntityAndInfoServer>) {
    let links = robot_set_entity_server.get_links_that_recently_gained_focus_ref().clone();
    for l in links {
        robot_set_entity_server.change_link_material_data(l.0, l.1, l.2, LynxMaterialType::Focus);
    }
    robot_set_entity_server.wipe_links_that_recently_gained_focus();

    let links = robot_set_entity_server.get_links_that_recently_left_focus_ref().clone();
    for l in links {
        robot_set_entity_server.reset_link_material_data(l.0, l.1, l.2);
    }
    robot_set_entity_server.wipe_links_that_recently_left_focus();
}
