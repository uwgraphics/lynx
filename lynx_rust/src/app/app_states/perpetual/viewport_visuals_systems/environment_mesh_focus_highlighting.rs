...
pub fn environment_mesh_focus_highlighting(mut env_entity_server: ResMut<EnvironmentEntityAndInfoServer>) {
    let obj1 = env_entity_server.get_env_objects_that_recently_gained_focus_ref().clone();
    for o in &obj1 {
        env_entity_server.change_env_object_material_data(*o, LynxMaterialType::Focus);
    }
    env_entity_server.wipe_environment_objects_that_recently_gained_focus();

    let obj2 = env_entity_server.get_env_objects_that_recently_left_focus_ref().clone();
    for o in &obj2 {
        env_entity_server.reset_env_object_material_data(*o);
    }
    env_entity_server.wipe_environment_objects_that_recently_left_focus();
}
