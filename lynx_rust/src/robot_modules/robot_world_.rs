...
pub struct RobotWorld {
    _robot_module_toolbox: Robot,
    _collision_environment: Option<CollisionEnvironment>
}

impl RobotWorld {
    pub fn new(robot_name: &str, configuration_name: Option<&str>, environment_name: Option<&str>) -> Result<Self, String> {
        let _robot_module_toolbox = Robot::new(robot_name, configuration_name)?;
    ...
    pub fn get_robot_module_toolbox_mut_ref(&mut self) -> &mut Robot {
        return &mut self._robot_module_toolbox;
    }

    pub fn get_robot_module_toolbox_ref(&self) -> &Robot {
