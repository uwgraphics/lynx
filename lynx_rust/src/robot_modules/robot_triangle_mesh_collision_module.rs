...
impl RobotTriangleMeshCollisionModule {
    ...
    fn _get_link_triangle_meshes_trimesh_engines(robot_configuration_module: &RobotConfigurationModule) -> Result<Vec<Option<TriMeshEngine>>, String> {
        ...
        let exists1 = check_if_path_exists_relative_to_robot_directory(robot_name.clone(), "autogenerated_metadata/link_triangle_meshes_visual".to_string());
        let exists2 = check_if_path_exists_relative_to_robot_directory(robot_name.clone(), "autogenerated_metadata/link_triangle_meshes_collision".to_string());
        if exists1 { file_choice = "visual"; }
        if exists2 && !exists1 { file_choice = "collision"; }
        if !exists1 && !exists2 { return Err("link triangle meshes were not accurately set, cannot make trimesh engines for them in triangle mesh collision module".to_string()) }

        let partial_fp = "autogenerated_metadata/link_triangle_meshes_".to_string() + file_choice;

        let file_names = get_all_files_in_directory_with_extension_relative_to_robot_directory( robot_name.clone(), partial_fp.clone(), "stl".to_string() );
        ...
        for i in 0..num_links {
            ...
            if found_idx == usize::MAX {
                out_vec.push(None);
            } else {
                let fp = get_path_to_particular_robot_directory(robot_name.clone()) + "/autogenerated_metadata/link_triangle_meshes_" + file_choice + "/" + file_names[found_idx].as_str();
