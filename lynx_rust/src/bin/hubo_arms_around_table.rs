...
fn main() -> Result<(), String> {
    let mut robot_configuration_module = RobotConfigurationModule::new("hubo".to_string(), Some("c1".to_string()));
    let mut robot_dof_module = RobotDOFModule::new(&robot_configuration_module);

    let mut collision_checker = RobotCollisionChecker::new_from_robot_configuration_module(&robot_configuration_module, None, Some("hubo_arms_around_table".to_string()));
    ...
    let q_goal = vec_to_dvec(&vec![0.0,-1.6,0.61261,-1.5,-0.7,0.,0.,3.14,-1.6,-0.61261,1.5,-0.7,0.,0.,-3.14]);

    let start = Instant::now();
    let res = Sprint::solve(&q_init, &q_goal, &mut collision_checker, &cspace_sampler, 0.05, false, false, None)?;
